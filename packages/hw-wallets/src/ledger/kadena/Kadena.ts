...
export default class Kadena extends Common {
  ...
  private async signTxInternal(
    ...
    if (isNaN_(params.amount)) throw new TypeError("amount is not a number");
    if (isNaN_(params.gasPrice)) throw new TypeError("gasPrice is not a number");
    if (isNaN_(params.gasLimit)) throw new TypeError("gasLimit is not a number");
    if (isNaN_(params.creationTime)) throw new TypeError("creationTime is not a number");
    if (isNaN_(params.ttl)) throw new TypeError("ttl is not a number");
    ...
    var hash = Buffer.from(hash_bytes).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''); // base64url encode, remove padding
